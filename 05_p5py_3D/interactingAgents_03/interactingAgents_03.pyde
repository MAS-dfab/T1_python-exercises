...
def setup():
    global bound, cam, cp5, font, a, g, flock, boids, ali, sep, coh, tGrid, bReset, tTrail
    #size(800, 800, OPENGL)
    fullScreen(P3D, 1)
    ...
    tTrail = createToggle("Trail", width-13*a, 2*a, 3*a, 3*a) #add trail toggle
    
    sAli = createSlider("Alignment", 2*a, 2*a, 12*a, a, 0, 100, 0) #new sliders
...
def draw():
    ...
    ali = cp5.getController("Alignment").getValue() #new slider values
    ...
    flock.run(ali, sep, coh, bound, dts, mxf, mxs) #incoorporate ali, sep, coh
