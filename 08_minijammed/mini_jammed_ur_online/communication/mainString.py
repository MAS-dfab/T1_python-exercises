...
def main():
    ...
    while True: # and ur and gh connected
        ...
        ### Gravel Placment ###
        tool_gravel = gh.wait_for_float_list() #7 client.send(MSG_FLOAT_LIST,tool_gravel_axis)
        ur.send_command_tcp(tool_gravel)
        ur.wait_for_ready()
        print("7: Set TCP Done")

        safe_in_pose_gravel_cmd = gh.wait_for_float_list() #8 client.send(MSG_FLOAT_LIST, safe_in_pose_gravel)
        ...
        pose_gravel_cmd = gh.wait_for_float_list() #9 client.send(MSG_FLOAT_LIST, pose_gravel)
        ...
        safe_out_pose_gravel_cmd = gh.wait_for_float_list() #10 client.send(MSG_FLOAT_LIST, safe_out_pose_gravel)
